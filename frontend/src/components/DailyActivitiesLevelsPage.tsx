...
interface DailyActivitiesLevelsPageProps {
  onStartLesson: (lessonId: number, lessonTitle: string) => void;
  onBack: () => void;
}

export function DailyActivitiesLevelsPage({
  onStartLesson,
  onBack,
}: DailyActivitiesLevelsPageProps) {
  // Path positions for 11 daily activities
  const pathPositions = [
    { top: '90%', left: '50%' }, // 1
    { top: '82%', left: '30%' }, // 2
    { top: '74%', left: '60%' }, // 3
  ...
  const lessons = [
    { id: 50, title: 'College', status: 'completed', stars: 3 },
    { id: 51, title: 'Work', status: 'completed', stars: 2 },
    { id: 52, title: 'Write', status: 'in-progress', stars: 0 },
    { id: 53, title: 'Call', status: 'locked', stars: 0 },
    { id: 54, title: 'Pray', status: 'locked', stars: 0 },
    { id: 55, title: 'Stand', status: 'locked', stars: 0 },
    { id: 56, title: 'Watch', status: 'locked', stars: 0 },
    { id: 57, title: 'Hang', status: 'locked', stars: 0 },
    { id: 58, title: 'Become', status: 'locked', stars: 0 },
    { id: 59, title: 'Assistance', status: 'locked', stars: 0 },
    { id: 60, title: 'Camp', status: 'locked', stars: 0 },
  ];
  ...
  return (
    <div className='flex-1 bg-accent/30 font-mono flex flex-col lg:flex-row overflow-hidden'>
      ...
      <div className='hidden lg:flex w-80 flex-col justify-center items-end p-8 z-10 relative'>
        <div className='bg-white/10 backdrop-blur-md text-white p-6 rounded-3xl shadow-xl relative w-full max-w-xs border border-white/20'>
          ...
          <p className='text-gray-200 font-medium leading-relaxed font-[VT323] text-xl'>
            Day in the Life! <br />
            <br />
            Master the signs for your daily routine and activities.
          </p>
      ...
      <div className='flex-1 h-full relative overflow-y-auto no-scrollbar'>
        <div className='w-full relative pb-16 min-h-full'>
          {/* Back Button Overlay */}
          <button
            onClick={onBack}
          ...
          >
            ...
            <span className='hidden md:inline font-[VT323] text-lg'>
              BACK TO MAP
            </span>
          </button>
          ...
          <div className='absolute top-4 right-0 left-0 text-center z-10 pointer-events-none'>
            <h1
              className='text-2xl md:text-3xl text-primary font-bold drop-shadow-[2px_2px_0_#000]'
              style={{ fontFamily: '"Press Start 2P"' }}
            >
              DAILY QUEST
            </h1>
          ...
          <img
            src={journeyBg}
            alt='Jungle Path Map'
            className='w-full h-auto max-h-[calc(100vh-8rem)] block select-none pointer-events-none filter hue-rotate-15 contrast-125 object-contain'
          />
      ...
      <div className='hidden lg:flex w-80 flex-col p-8 z-10 gap-8 h-full justify-center'>
        ...
        <div className='bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 p-6 shadow-xl'>
          ...
          <div className='flex justify-between items-center mb-2'>
            ...
            <span className='text-primary font-bold'>160 XP</span>
          </div>
          <div className='flex justify-between items-center'>
            <span className='text-white'>Streak</span>
            <span className='text-orange-500 font-bold'>13 Days</span>
          </div>
        </div>
