...
interface GreetingsLevelsPageProps {
  onStartLesson: (lessonId: number, lessonTitle: string) => void;
  onBack: () => void;
}

export function GreetingsLevelsPage({
  onStartLesson,
  onBack,
}: GreetingsLevelsPageProps) {
  // Path positions for 8 greetings (winding upwards)
  const pathPositions = [
    { top: '88%', left: '50%' }, // 1
    { top: '78%', left: '30%' }, // 2
    { top: '68%', left: '60%' }, // 3
  ...
  const lessons = [
    { id: 30, title: "I'm Good", status: 'completed', stars: 3 },
    { id: 31, title: 'I want Food', status: 'completed', stars: 2 },
    { id: 32, title: 'All_Gone', status: 'in-progress', stars: 0 },
    { id: 33, title: 'ok fine', status: 'locked', stars: 0 },
    { id: 34, title: 'not fine', status: 'locked', stars: 0 },
    { id: 35, title: 'See', status: 'locked', stars: 0 },
    { id: 36, title: 'Stop', status: 'locked', stars: 0 },
    { id: 37, title: 'How_Many', status: 'locked', stars: 0 },
  ];
      ...
      <div className='hidden lg:flex w-80 flex-col justify-center items-end p-8 z-10 relative'>
        <div className='bg-white/10 backdrop-blur-md text-white p-6 rounded-3xl shadow-xl relative w-full max-w-xs border border-white/20'>
          ...
          <p className='text-gray-200 font-medium leading-relaxed font-[VT323] text-xl'>
            Social Butterfly! <br />
            <br />
            Learn these essential phrases to start communicating with the world.
          </p>
